...
/* useTheme, Topbar, AdminPanel, HistoryPanel definitions stay unchanged (omitted for brevity) */
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-b from-background to-background/60 text-foreground">
      <div className="flex h-[100dvh]">
        ...
        {leftOpen && (
          <div className="hidden sm:block w-72 relative">
            <MegaSidebar t={t} agents={agents} selectedAgentId={selectedAgentId} onSelectAgent={setSelectedAgentId} onToggleAgent={handleToggleAgent} onRefresh={handleRefreshStatuses} glowMode={glowMode} />
            <button className="absolute -right-3 top-1/2 -translate-y-1/2 bg-background border rounded-full w-6 h-6 flex items-center justify-center" onClick={() => setLeftOpen(false)} title="Collapse">
        ...
        {rightOpen && (
          <div className="hidden sm:block w-72 relative">
            <button className="absolute -left-3 top-1/2 -translate-y-1/2 bg-background border rounded-full w-6 h-6 flex items-center justify-center z-10" onClick={() => setRightOpen(false)} title="Collapse">
              <PanelRight className="w-4 h-4" />
            </button>
            <RightSidebar t={t} agents={agents} selectedAgentId={selectedAgentId} onSelectAgent={setSelectedAgentId} onToggleAgent={handleToggleAgent} onRefresh={handleRefreshStatuses} glowMode={glowMode} />
          </div>
        )}
        {!rightOpen && (
          <div className="hidden sm:flex w-3 items-center justify-center">
