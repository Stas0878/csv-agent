  ...
  return (
    ...
    <div className={`w-screen h-screen overflow-hidden bg-gradient-to-b from-background to-background/60 text-foreground ${embedClass}`}>
      <div className="flex h-full">
        ...
        {/* Swap sidebars if configured */}
        {uiConfig.layout.swapSidebars && (
          <>
            {/* Move right sidebar to left position by swapping rendering order visually for lg+ */}
          </>
        )}
        ...
        <div className="flex-1 min-w-0 min-h-0 flex flex-col">
          ...
          <div className="flex-1 min-h-0 overflow-hidden p-3 md:p-4">
            ...
            <TabsList className="md:hidden mb-3">
              {uiConfig.tabs.order
                .filter(k => uiConfig.tabs.enabled?.[k] !== false)
                .map(k => (
      ...
      {adminOpen && (
        <>
          ...
          <DraggableBlock id="inputComposer" positions={dragPositions} updatePosition={updateDragPos}>
            <div className="bg-card/70 border rounded p-3 w-96 shadow">Строка ввода (demo)</div>
          </DraggableBlock>
          <DraggableBlock id="previewOverlay" positions={dragPositions} updatePosition={updateDragPos}>
