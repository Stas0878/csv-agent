...
function App() {
  ...
  // initial load with zod validation
  useEffect(() => {
    (async () => {
      try {
        const a = await getAgents();
        const safe = AgentsSchema.safeParse(a);
        if (safe.success) setAgents(safe.data); else toast({ title: "Agents", description: "Invalid data" });
      } catch (e) { toast({ title: "Agents", description: "Failed to load" }); }
      try {
        const h = await getHistory();
        const safeH = HistorySchema.safeParse(h);
        if (safeH.success) setHistory(safeH.data); else toast({ title: "History", description: "Invalid data" });
      } catch (e) { /* ignore */ }
    })();
  }, []);
